...
export function ProductForm({ product, categories }: { product?: ProductView | null; categories: CategoryView[] }) {
  const initialColors = useMemo<ColorState[]>(() => {
    ...
    return product.colors.map((color, index) => ({
      ...
      imageUrl: color.imageUrl || product.images.find((image) => image.colorId === color.id)?.url || "",
      isAvailable: color.isAvailable,
      sizes: color.variants.filter((variant) => variant.isAvailable).map((variant) => variant.size)
    }));
  }, [product]);
  ...
  function toggleSize(index: number, size: string) {
    setColors((current) =>
      current.map((color, colorIndex) => {
        ...
        const sizes = color.sizes.includes(size) ? color.sizes.filter((item) => item !== size) : [...color.sizes, size];
        return { ...color, sizes };
      })
    );
  }
  ...
  async function uploadColorImage(index: number, file?: File) {
    ...
    updateColor(index, { imageUrl: data.url });
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!body.nameAr || !body.nameEn || !body.internalCode || !body.categoryId || Number(body.price) <= 0) {
    ...
    if (!response.ok) {
      setStatus(data?.error || "تعذر حفظ المنتج. تأكد من الحقول وقاعدة البيانات.");
      return;
    }
    ...
    <form onSubmit={submit} className="grid gap-6">
      <section className="premium-card grid gap-4 p-4 sm:p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field name="nameAr" label="اسم المنتج بالعربية" defaultValue={product?.nameAr} required onChange={updateName} />
          <Field name="nameEn" label="اسم المنتج بالإنجليزية" defaultValue={product?.nameEn} required onChange={updateName} />
          <Field
            name="slug"
          ...
          />
          <Field name="internalCode" label="الكود الداخلي للموظفين" defaultValue={product?.internalCode} required />
          <label className="block">
            <span className="mb-2 block text-sm font-black">القسم</span>
            <select className="admin-field" name="categoryId" defaultValue={product?.categoryId || categories[0]?.id} required>
          ...
          <Field name="price" label="السعر بالليرة السورية" type="number" min={1} defaultValue={String(product?.price ?? "")} required />
          <label className="block">
        ...
        <Textarea name="shortDescriptionAr" label="وصف قصير بالعربية" defaultValue={product?.shortDescriptionAr} />
        <Textarea name="shortDescriptionEn" label="وصف قصير بالإنجليزية" defaultValue={product?.shortDescriptionEn} />
        <Textarea name="descriptionAr" label="وصف كامل بالعربية" defaultValue={product?.descriptionAr} required />
        <Textarea name="descriptionEn" label="وصف كامل بالإنجليزية" defaultValue={product?.descriptionEn} required />
      </section>
      ...
      <section className="premium-card grid gap-4 p-4 sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">ارفع صورة لكل لون حتى تتغير صورة المنتج عند اختيار اللون في المتجر.</p>
          </div>
          <button
            type="button"
            className="tap-target rounded-full bg-ink px-5 py-3 text-sm font-black text-bone"
            onClick={() => setColors((current) => [...current, emptyColor(current.length)])}
          >
            إضافة لون
          </button>
        </div>
        ...
        <div className="grid gap-4">
          {colors.map((color, index) => (
            <div key={color.tempId} className="rounded-2xl border border-ink/10 bg-bone p-4">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <span className="h-10 w-10 rounded-full border border-ink/15" style={{ backgroundColor: color.hex }} />
                  <strong>لون {index + 1}</strong>
                </div>
                <button
                  type="button"
                  className="rounded-full border border-ink/15 px-4 py-2 text-sm font-black"
                  onClick={() => setColors((current) => (current.length > 1 ? current.filter((_, itemIndex) => itemIndex !== index) : current))}
                >
                  حذف
                </button>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="اسم اللون بالعربية" value={color.nameAr} onChange={(event) => updateColor(index, { nameAr: event.target.value })} required />
                <Field label="اسم اللون بالإنجليزية" value={color.nameEn} onChange={(event) => updateColor(index, { nameEn: event.target.value })} required />
                <Field label="رمز اللون HEX" value={color.hex} onChange={(event) => updateColor(index, { hex: event.target.value })} pattern="^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$" required />
                <label className="flex items-center gap-2 self-end rounded-2xl bg-paper p-4 font-black">
                  <input
                    type="checkbox"
                    checked={color.isAvailable}
                    onChange={(event) => updateColor(index, { isAvailable: event.target.checked })}
                  />
                  اللون متاح
                </label>
              </div>
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-[11rem_1fr]">
                <div className="relative aspect-square overflow-hidden rounded-2xl bg-paper">
                  {color.imageUrl ? (
                    <Image src={color.imageUrl} alt={color.nameAr || "صورة اللون"} fill className="object-cover" />
                  ) : (
                ...
                <div className="grid content-start gap-4">
                  <label className="block rounded-2xl border border-dashed border-ink/20 bg-paper p-4">
                    <span className="mb-2 block text-sm font-black">رفع صورة لهذا اللون</span>
                    <input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={(event) => uploadColorImage(index, event.target.files?.[0])} />
                  </label>
                  <div>
                      ...
                      {sizeOptions.map((size) => (
                        <button
                          ...
                          className={`tap-target rounded-full px-4 py-2 text-sm font-black ${
                            color.sizes.includes(size) ? "bg-ink text-bone" : "bg-paper text-ink"
                          }`}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <button className="tap-target rounded-full bg-ink px-7 py-4 font-black text-bone">حفظ المنتج</button>
